...
logBtn.addEventListener('click', async () => {
  ...
  fetch(`${BACKEND_BASE}/log_page`, {
    ...
    .then(res => res.json())
    .then(data => {
      logBtn.textContent = 'Log Page';
      logBtn.disabled = false;
      resultsDiv.textContent = 'Page logged!';
      listIndexedPages();
    })
    .catch(() => {
      logBtn.textContent = 'Log Page';
      logBtn.disabled = false;
      resultsDiv.textContent = 'Error logging page.';
    });
});
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const logBtn = Array.from(btns).find(b => b.textContent.trim() === 'Log Page');
  if (logBtn) {
  ...
  const askBtn = Array.from(btns).find(b => b.textContent.trim() === 'Ask');
  if (askBtn) {
